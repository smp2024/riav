...
class RecyclingController extends Controller
{
    ...
    public function getHome($status)
    {
        ...
        switch ($status) {
            ...
            case '1':
                $products = Article::where('status', '1')->where('module', 'reciclaje')->orderBy('id', 'DESC')->paginate(20);
                break;
            case '0':
                $products = Article::where('status', '0')->where('module', 'reciclaje')->orderBy('id', 'DESC')->paginate(20);
                break;
            case 'trash':
                $products = Article::onlyTrashed()->where('module', 'reciclaje')->orderBy('id', 'DESC')->paginate(20);
                break;
    ...
    {
        $cats = Article::where('module', 'reciclaje')->pluck('name', 'id');
        $data = [
            'cats' => $cats
        ];
    ...
    public function postArticleAdd(Request $request)
    {
        ...
        if($validator->fails()):
            ...
            if($product->save()):
                ...
                return redirect('/admin/exhibitions/1')->with('message', ' Reciclaje guardado con éxito.')->with('typealert', 'success');
    ...
    public function postArticleEdit(Request $request, $id)
    {
        ...
        if($validator->fails()):
            ...
            if($product->save()):
                ...
                return back()->with('message', ' Reciclaje actualizada con éxito.')->with('typealert', 'success');
    ...
    public function getArticleDelete($id)
    {
        ...
        if($product->delete()):
            ...
            return back()->with('message', ' Reciclaje enviado con éxito a la papeplera.')->with('typealert', 'success');
