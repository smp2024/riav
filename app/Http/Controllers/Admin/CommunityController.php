...
class CommunityController extends Controller
{
    ...
    public function getHome($status)
    {
        ...
        switch ($status) {
            ...
            case '1':
                $products = Article::where('status', '1')->where('module', 'comunidad')->orderBy('id', 'DESC')->paginate(20);
                break;
            case '0':
                $products = Article::where('status', '0')->where('module', 'comunidad')->orderBy('id', 'DESC')->paginate(20);
                break;
            case 'trash':
                $products = Article::onlyTrashed()->where('module', 'comunidad')->orderBy('id', 'DESC')->paginate(20);
                break;
    ...
    {
        $cats = Article::where('module', 'comunidad')->pluck('name', 'id');
        $data = ['cats' => $cats];
    ...
    public function postArticleAdd(Request $request)
    {
        ...
        if($validator->fails()):
            ...
            if($product->save()):
                ...
                return redirect('/admin/communities/1')->with('message', ' Artículo guardado con éxito.')->with('typealert', 'success');
    ...
    public function postArticleEdit(Request $request, $id)
    {
        ...
        if($validator->fails()):
            ...
            if($product->save()):
                ...
                return back()->with('message', ' Comunidad actualizada con éxito.')->with('typealert', 'success');
    ...
    public function getArticleDelete($id)
    {
        ...
        if($product->delete()):
            ...
            return back()->with('message', ' Comunidad enviada con éxito a la papeplera.')->with('typealert', 'success');
    ...
    public function getArticleRestore($id)
    {
        ...
        if ($product->save()):
            ...
            return redirect('/admin/community/'.$product->id.'/edit')->with('message', ' La notícia se restauro correctamente.')->with('typealert', 'success')->withInput();
